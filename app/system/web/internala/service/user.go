...
//	}
//	// 赋予权限
//	if _, err = dao.ReUserRole.Insert(g.Map{
...
func (u *userService) ListCodingTimeByUserId(ctx context.Context, req *define.ListCodingTimeByUserIdReq) (resp []*define.CodingTimeRecord, err error) {
	...
	d := dao.CodingTime.Ctx(ctx)
	// 限定范围
	if req.CourseId != 0 {
		labIds, err := dao.Lab.Ctx(ctx).Where(dao.Lab.Columns.CourseId, req.CourseId).Array(dao.Lab.Columns.LabId)
		if err != nil {
			return nil, err
		}
		d = d.Where(dao.CodingTime.Columns.LabId, labIds)
	}
	d = d.Where(dao.CodingTime.Columns.UserId, req.UserId)
	d = d.Fields(fmt.Sprintf("SUM(%s)", dao.CodingTime.Columns.Duration),
		fmt.Sprintf("Date_Format(%s,'%%Y-%%m-%%d') as %s", dao.CodingTime.Columns.CreatedAt, dao.CodingTime.Columns.CreatedAt)).
		Group(fmt.Sprintf("Date_Format(%s,'%%Y-%%m-%%d')", dao.CodingTime.Columns.CreatedAt))
